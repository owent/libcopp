...
class stackful_channel_context_base {
 ...
 private:
  ...
  // matches the C++20 coroutine backend's promise_caller_manager and is the least surprising
  // semantics. The list keeps the wake order and allows O(1) removal by iterator, while the hash
  // index keeps dedup-on-add and passive removal O(1) as well.
