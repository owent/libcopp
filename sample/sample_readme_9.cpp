...
struct sample_generator_waker_t {
    ...
    sample_generator_waker_t(int32_t c, int32_t at) : code(c), await_times(at) { refer_to = g_sample_generator_waker_list.end(); }
...
static void poll_no_trival_task_and_timeout() {
    ...
    std::cout << "\ttask " << t1.get_task_id() << " status should be DONE(" << static_cast<int>(sample_task_t::status_type::DONE)
              << "), real is: " << static_cast<int>(t1.get_status()) << std::endl;
    if (t1.data() && t1.data()->is_error()) {
        std::cout << "\texpected code: " << SAMPLE_TIMEOUT_ERROR_CODE << ", real is: " << *t1.data()->get_error() << std::endl;
    }
