...
if [[ "$1" == "format" ]]; then
  ...
  bash cmake_dev.sh -lus -b Debug -r build_jobs_coverage -c $USE_CC -- "-DCMAKE_C_FLAGS=$GCOV_FLAGS"  \
  ...
  bash cmake_dev.sh -lus -b Debug -r build_jobs_ci -c $USE_CC -- ${PROJECT_ADDON_OPTIONS[@]};
  cd build_jobs_ci ;
  cmake --build . -j2 || cmake --build .;
